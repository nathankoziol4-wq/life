...
const CORE: Feature[] = [
  ...
  f('Vie/Personnalité/Habitudes qui coûtent', 'COMPLETE', { src: 'systems/psyche.ts#advanceHabits', ui: 'components/PersonalityPanel.tsx', pers: 1, cons: 1, test: 'personnalite', deps: ['Finance', 'Santé'], impact: 3, note: 'ce qu’elles coûtaient était faux — la fréquence montait d’une douzaine d’occurrences par an sans fin, jusqu’à 98 heures de loisirs par semaine à cinquante ans et un temps libre réduit à 1,4 ; bornée là où l’effet cesse de croître, elle rend 23 à 39 heures et un coût de 0,078 du revenu au lieu de 0,276' }),
  f('Vie/Personnalité/S’y mettre, ou renoncer', 'COMPLETE', { src: 'systems/psyche.ts#quitHabit', ui: 'screens/CharacterScreen.tsx', pers: 1, cons: 1, test: 'personnalite', deps: ['Vie/Personnalité/Habitudes qui coûtent'], impact: 3, note: 'renoncer n’est pas un interrupteur : la ténacité résiste, et elle vaut enfin quelque chose — 95 % des habitudes valaient exactement 100, si bien qu’arrêter de lire était aussi dur qu’arrêter de fumer ; ancrée sur ce que l’habitude est, elle rend 42 % de chances pour les réseaux contre 68 % pour apprendre' }),
  f('Vie/Personnalité/Une habitude a un plafond', 'COMPLETE', { internal: 1, src: 'data/habits.ts#HABIT_CEILING', ui: 'components/PersonalityPanel.tsx', pers: 1, cons: 1, test: 'personnalite', deps: ['Santé'], impact: 3, note: '`applyHabitEffects` bornait déjà l’effet à 1,4 fois la fréquence de référence, mais rien ne bornait la fréquence : au-delà, l’habitude ne rendait plus rien et continuait de prendre du temps et de l’argent — la même borne s’écrit désormais des deux côtés' }),
  f('Vie/Personnalité/Ambitions', 'COMPLETE', { src: 'systems/psyche.ts#setAmbition', ui: 'screens/CharacterScreen.tsx', pers: 1, cons: 1, test: 'personnalite', deps: ['Vie/Satisfaction'], impact: 3, note: 'on s’en fixe une, et elle prend la place d’une autre — quatre au plus, et la mesure disait qu’on y était déjà (98 % des vies, quatre en médiane) ; ce qu’on laisse laisse un regret' }),
  f('Vie/Personnalité/Un but qu’on ne poursuit pas s’éteint', 'COMPLETE', { internal: 1, src: 'data/ambitions.ts#NEGLECTED', ui: 'components/PersonalityPanel.tsx', pers: 1, cons: 1, test: 'personnalite', deps: ['Vie/Satisfaction'], impact: 3, note: 'le poids dérivait vers l’accord avec ses valeurs, mesuré de 15,9 à 46,5 sur 780 couples — jamais sous le seuil d’extinction de 12, donc rien ne s’éteignait jamais et le regret promis par `advanceAmbitions` était inatteignable ; la cible est désormais rabattue par l’inaction : 39 % de survie à progression nulle contre 95 % dès qu’on avance' }),
