...
La source unique de vérité de ce skill. **Ce bloc a été remplacé pour ce
dépôt.** L'auteur d'origine prévoit explicitement cette adaptation : « To adapt
this skill to a different design system, edit only the Design Values section.
The entire audit follows it automatically. » Les valeurs d'origine visaient
Tailwind ; Odyssia n'a pas Tailwind et tient son propre système dans
`src/ui/theme/tokens.css`. Toute valeur ci-dessous en est tirée telle quelle.

**Aucune valeur ne s'écrit dans un écran.** Le fichier de jetons porte sa
raison d'être en tête : il a été écrit après avoir mesuré 443 `style={{}}`
posés à la main. Il y en a 575 aujourd'hui, dans 62 fichiers — 55 tailles de
police et 74 valeurs de pixels. C'est la cible principale de tout audit ici :
remplacer une valeur en dur par son jeton, jamais l'inverse.

**Polices :** `Inter` avec la pile système en repli, et `--font-numeric` pour
les chiffres. *L'audit ci-dessus demande de remplacer Inter — pas ici : c'est
un choix assumé du dépôt, et le jeton `--font` est le seul endroit qui en
décide.*

**Graisses :** 400 · 550 · 680 · 820. Rien d'autre.

**Échelle de texte** — des rôles nommés par ce qu'ils font, pas des tailles :

| Jeton | Taille | Usage |
|---|---|---|
| `--text-display` | 42px | un nombre qu'on regarde |
| `--text-title` | 27px | le titre d'un écran |
| `--text-heading` | 20px | un intertitre, l'émoji d'une ligne |
| `--text-body` | 15px | le corps |
| `--text-sub` | 13,5px | le sous-titre d'une ligne |
| `--text-section` | 13px | l'intertitre de section, majuscules, suivi 0,09em |
| `--text-caption` | 12px | la légende |
| `--text-input` | 16px | **un champ de saisie, et cela ne se négocie pas** — en dessous, Safari agrandit la page et ne la réduit jamais |

**Interlignes :** 1,15 (serré) · 1,3 (courant) · 1,5 (corps).
**Suivi :** −0,03em (display) · −0,02em (titre) · +0,09em (section).

**Espacement :** 0 · 4 · 8 · 12 · 16 · 20 · 24 · 32 · 40 · 48 · 64. Une
progression de 4, la seule autorisée.

**Rayons :** 8 (xs) · 12 (sm) · 18 (base) · 26 (lg) · 34 (xl) · 999 (pilule).
Pas de valeurs Tailwind, et pas de formule de rayon imbriqué : l'échelle
suffit.

**Structure :** barre de navigation 72px · en-tête 96px · **cible tactile
minimale 44px** · largeur maximale de l'application 480px. Marges sûres par
`env(safe-area-inset-*)`.

**Matière (thème clair) :** fond `#f4f5fb` · profond `#e8eaf4` · surface
`#ffffff` · alternée `#f8f9fd` · creusée `#eceffa`. Encre `#14162a` · douce
`#474b68` · sourde `#7b8099` · pâle `#a8adc2`. Traits `#e4e6f1` et `#d0d4e4`.
Le thème sombre redéfinit les mêmes jetons — un écran ne le sait jamais.

**Le sens — sept familles, et pas une de plus.** Chacune a un ton plein pour
le trait, l'icône ou le chiffre, et un ton doux pour le fond d'une pastille :
`--argent` `#0e8f63` · `--sante` `#d4405f` · `--amour` `#d4357f` ·
`--carriere` `#2f6fd0` · `--savoir` `#7a49c8` · `--crime` `#3f4667` ·
`--gloire` `#c07a12`. Plus `--primary` `#4a3ce0` et les états `--good`,
`--bad`, `--warn`, `--accent`. *Les noms disent la vie, pas la couleur : un
écran qui écrirait « vert » se tromperait le jour où le vert change.* Une
interface qui colore tout ne colore rien.

**Icônes :** des émoji, et c'est délibéré — `Row` a un champ `emoji`, la barre
d'onglets aussi. *L'audit ci-dessus demande de remplacer Lucide, Feather ou
Material par un jeu approuvé ; ici il n'y a pas de jeu d'icônes à remplacer.*

**Mouvement :** 90ms (instantané) · 160ms (vif) · 240ms (courant) · 380ms
(lent). Courbes : `--ease-out` `cubic-bezier(0.22, 1, 0.36, 1)` ·
`--ease-in-out` `cubic-bezier(0.65, 0, 0.35, 1)` · `--ease-spring`
`cubic-bezier(0.34, 1.56, 0.64, 1)`. L'animation ne doit jamais faire
attendre.

**Le vocabulaire de listes** (`src/ui/components/list.tsx`) — quatre pièces
portent tout le jeu, et un écran n'en dessine pas d'autres :

- `Section` → `Card` → `Row`. La ligne fait 44px au minimum, 12/16 de marge
  intérieure, 12 d'écart, un trait sous chacune sauf la dernière.
- Une ligne **fermée** affiche sa raison (`because`) **à la place** du
  sous-titre, et **perd son clic** — on ne peut donc pas y mettre un geste :
  il va dans `right`, en bouton. Elle ne pose jamais `disabled`, qui la
  retirerait de l'arbre d'accessibilité : « indisponible » veut presque
  toujours dire « pas encore, et voilà pourquoi », et cette raison *est* le
  contenu.
- Liserés de carte : `--good` ce qui va bien · `--warn` ce qui demande une
  réponse · `--bad` ce qui a mal tourné. Un liseré, pas un aplat.
