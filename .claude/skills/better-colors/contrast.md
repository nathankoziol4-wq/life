...
> **Note pour ce dépôt (Odyssia).** L'outillage existe déjà, ne le réécris pas :
> `npm run audit:contraste` (`tools/contraste.mjs`) lit `src/ui/theme/tokens.css`
> et tabule les quatre encres sur les cinq fonds réels — `surface`,
> `surface-alt`, `surface-sunken`, `bg`, `bg-deep` — dans les deux thèmes.
> `src/engine/__bench__/contraste.test.ts` en fait un garde-fou : planchers
> WCAG, plus une vérification que l'encre pâle ne reprend pas de texte et que
> les quatre encres restent distinctes.
>
> Le paragraphe ci-dessus décrit exactement l'erreur commise ici : une première
> mesure ne regardait que le blanc, le fond le plus favorable, et concluait à un
> manque de 0,6 point. Sur `--bg-deep` il en manquait 1,25. C'est pourquoi
> l'outil énumère les cinq fonds au lieu d'un.
>
> Le correctif appliqué suit la règle « Change lightness first » : `--ink-muted`
> a perdu 9 % de clarté à teinte et saturation constantes. Les planchers tenus
> aujourd'hui sont WCAG (4,5:1 pour le texte, 3:1 pour le reste) ; l'APCA n'est
> pas calculé par l'outil, et s'en servir comme départage demanderait de
> l'ajouter d'abord.
