...
> **Note pour ce dépôt (Odyssia).** Le jeu n'a pas de jeu d'icônes vectorielles :
> il emploie l'emoji, délibérément et partout — `Row` prend une prop `emoji`, et
> le vocabulaire est cohérent d'un écran à l'autre (⚖️ pour une procédure, 💰
> pour de l'argent, 🕰️ pour du temps donné). Ce n'est pas un défaut à corriger
> ni un raccourci : c'est le parti pris visuel du projet, et un outil de revue
> l'a déjà signalé deux cents fois à tort.
>
> Les règles de trait, de graisse et de grille 24px ci-dessous sont donc **sans
> objet ici** — un emoji n'a pas d'épaisseur de trait. Ce qui reste applicable :
> la taille relative au texte, l'alignement optique, la direction (miroir en
> RTL), et surtout ce que `break/scenarios.md` retient de l'emoji — sauts de
> hauteur de ligne, centrage vertical cassé, troncature qui coupe un caractère
> en deux. Ce sont de vrais défauts, et eux valent d'être cherchés.
